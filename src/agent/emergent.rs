...
pub fn compute_view_confidence(ctx: &TaskCtx) -> ViewConfidence {
    compute_view_confidence_at(ctx, std::time::Instant::now())
}

/// [`compute_view_confidence`] with the clock supplied, the way `SwimMembership` has always taken
/// its `now`.
///
/// Injecting `now` is how a test reaches the stale branch at all — the alternative was a branch
/// nothing could exercise. Added while the peer table's stamps were briefly `u64`, and kept when
/// they went back to `Instant`: the testability it buys is the point, not the representation.
pub fn compute_view_confidence_at(ctx: &TaskCtx, now: std::time::Instant) -> ViewConfidence {
    ...
    for (_id, last_seen) in guard.iter() {
        let age = mycelium_core::sim_seam::mono_span(last_seen, &now);
        if age <= HEARD_WINDOW {
            peers_heard += 1;
...
mod tests {
    ...
    ///
    /// This branch is reachable only by injecting `now` — which is why `compute_view_confidence_at`
    /// exists. Without it the staleness comparison could have been stuck at "always fresh" and the
    /// suite would have agreed.
    #[test]
    fn view_confidence_does_not_count_a_peer_last_heard_before_the_window() {
        ...
        let inside = heard_at + HEARD_WINDOW - Duration::from_secs(1);
        ...
        let outside = heard_at + HEARD_WINDOW + Duration::from_secs(1);
