...
per-lock gauge** (cardinality); inspect an individual lock with `GET /consensus/lock/{name}` (the route
captures the path tail, so the hierarchical slot is typed as-is; bearer when a token is set —
`consensus:read`).
