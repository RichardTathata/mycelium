...
**What "in-process" left open, and where it was closed.** Process isolation and a real network severance are
the two-mesh **Docker** suite's claim (row 10b, landed the same day): one container per node, and the
federation link cut with `docker network disconnect` rather than by stopping a gateway. Run it with
`make test-federation`; CI gates it as the `federation` job. With that, **the gate is met without a caveat** —
the same choreography and the same three legs of non-merger, over real processes and a real severance.

What is still *not* claimed, stated so the gate is not read as more than it is. The catalogue reply is signed
and bound to its asker when the edge holds the domain's key (10a); a client that does not hold the partner's
key still relies on it only as an observation attributed to the gateway asked. Streaming is refused rather than
