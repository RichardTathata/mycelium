...
| every `tokio::select!` in `connection.rs`, `writer.rs`, `tasks.rs`, `swim.rs`, `signal.rs`, the governors, `mcp.rs`, `lifecycle.rs` | **which ready branch wins** | **scheduler seam**: the kernel decides readiness order; exploration permutes it. **Measured 2026-09-18 (CN2):** a *whole node* recorded under the kernel — a linearizable award on a started `GossipAgent`, same identity for both runs — diverges at seq 7: the recording has the membership governor's `rng jitter` draw where the replay has the award round's `consensus/defer` timer. The two tasks' interleaving is this row's, and until it is owned, a single task's effects replay (scenario A, the corpus) and a node's do not. Pinned as a gap by `mycelium-commitment`'s `a_whole_node_recording_of_a_linearizable_award_diverges_without_a_scheduler_seam`, which asserts the divergence and names it, so that landing this seam flips it into the claim |
