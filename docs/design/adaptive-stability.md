...
| **5** ✓ *(2026-09-18)* | **admission control at the companions' queues, reported.** The tuple space's watermark already refused a `put`; the refusal was a silence. Now each stage counts `rejected_total`, the primary reports it beside `admitted` and `taken` (`TupleSpace::admission` — `Some` at the primary, `None` elsewhere, because the depth RPC's fixed encoding cannot grow and §3 says the primary owns the deficit) and the metrics writer publishes it. The blackboard gets its first bound, `BoardConfig.high_watermark: Option<u64>` (`None` = unbounded, as before), refusing `post` with a counted `Backpressure` that crosses the RPC as itself; replication and replay never refuse. Both bounds are Tier B — self-imposed, two-step, not hard. Example `mycelium-tuple-space/examples/admission`; runbook `docs/operations/admission-control.md`; §6.6 names `BoardConfig` |
