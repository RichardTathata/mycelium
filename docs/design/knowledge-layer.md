...
| **6** ✓ | **the demonstration — the contract completes here.** `src/knowledge/gate.rs` carries §8's three negative cases as a named, separately-runnable gate (`make gate-knowledge`, its own CI step) **plus two positive controls**; `examples/knowledge_layer.rs` runs the lifecycle end to end and closes by printing what it does *not* demonstrate |
| **7** ✓ | **adapters.** The trace adapter (`mycelium-reason/src/knowledge.rs`, feature `knowledge`): a `TraceEvent` as an **observation**, `derived_from` links only where the caller asserts them, and a batch form that is link-free *by construction* (§6, made structural). The AgentFacts adapter (`mycelium-agentfacts/src/knowledge.rs`): a *verified* self-signed document as a **claim** issued by its signing *key*, which §3's resolution can never count as evidence — the record-type split enforced at the boundary. Needed `mycelium::hlc` public (additive re-export; `mycelium-core` already committed to it) |
