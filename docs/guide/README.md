...
| [17](17-federation.md) | **Federation** — cross-domain discovery with self-certified AgentFacts, **plus the v3 invoke edge** (`mycelium::federation`: descriptors, filtered catalogs, call credentials, budgets, partition/reconnect — contract only, no transport yet): serve and pull the edge document, verify a peer domain without a shared CA, and the multi-author domain board | `cargo run -p mycelium-coop-examples --bin federation_facts` | 10 min |
