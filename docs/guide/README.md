...
| [15](15-reasoning-and-langgraph.md) | **Reasoning & LangGraph** — LangGraph *on* Mycelium: routed inference (with the OpenAI-compatible endpoint any client can point at), fleet traces, the checkpointer, and the deploy/reheal flagship (a model that follows its thread across node death) | the [`examples/langgraph/`](../../examples/langgraph/) ladder | 15 min |
...
| [17](17-federation.md) | **Federation** — cross-domain discovery with self-certified AgentFacts, **plus the v3 invoke edge** (`mycelium::federation`: descriptors, filtered catalogs, call credentials, budgets, partition/reconnect — **and its transport**, shipped in v2.8.0: the credential as one header on `/a2a`, the signed catalogue, and a two-mesh suite proving non-merger with the link cut at the network layer): serve and pull the edge document, verify a peer domain without a shared CA, and the multi-author domain board | `cargo run -p mycelium-coop-examples --bin federation_facts` | 10 min |
...
| [24](24-commitments.md) | **Commitments — the contract net** — the third coordination model beside chapter 14's tuple space and blackboard: routing by *negotiation*, why an award is a receipt-bearing operation rather than a write, and why a pure award rule lets an award be checked rather than trusted | `cargo run -p mycelium-commitment --example redistribution_cn` | 12 min |
