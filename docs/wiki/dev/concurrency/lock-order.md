...
| 24 | `TupleStore::inflight` / per-stage `StageState::inner` (`StageInner`) / `StageState::queue_waits_us` | `parking_lot::Mutex<…>` | take/complete/requeue/sweep + latency sampling (`mycelium-tuple-space/src/store.rs`) | Store-internal leaves, sync methods only. The per-stage `inner` is the exactly-once **claim** lock (single-owner take); `inflight` is the crash-requeue registry. Never held while acquiring another row |
| 25 | tuple-space `WalWriter::inner` | `parking_lot::Mutex<WalInner>` | WAL append / checkpoint / replay (`mycelium-tuple-space/src/store.rs`, `struct WalWriter`) | Leaf; short synchronous file ops under the guard (the WAL is single-writer by construction) |
...
| 28 | blackboard `WalWriter::inner` | `parking_lot::Mutex<WalInner>` | WAL append / checkpoint (`mycelium-blackboard/src/wal.rs`, `struct WalWriter`) | Leaf; same single-writer discipline as row 25 |
...
| 36 | `InferenceRouter::inflight` | `std::sync::Mutex<HashMap<String, u32>>` | `reserve` / `Reservation::drop` / `inflight` / `candidates` (`mycelium-reason/src/route.rs`) | Leaf, µs map read/write, never held across an await (`candidates` clones a snapshot then ranks). Added 2026-09-04: the router's **local reservations** — open calls per provider, weighted into the rank to cover the pheromone's propagation lag. Node-local by design (never gossiped, never a pheromone) |
| 37 | `InstallRights::ledger` | `Arc<tokio::sync::Mutex<RightsLedger>>` | `admit_install` (`try_lock`, a view read on the synchronous admission path — a busy ledger is a refusal, never a wait), the spawned tasks that record a refusal (`admit`, an fsynced append) and publish the head (`mycelium-wasm-host/src/provisioner.rs`, item 4 PR 4c) | Leaf; released before `hosted` (row 21) is taken — `start_install` reads the count, then the ledger, then reserves, never nested. Held across the journal's `await` only in the spawned tasks, which hold nothing else |
| 38 | `FederationEdge::trust` | `std::sync::RwLock<EdgeTrust>` (policy + trust bundle) | `authenticate` / `authorize` / `catalog_for` (read, on every federated request), `set_policy` / `revoke` (write, operator actions) (`src/federation/edge.rs`, item 2 PR 8) | Leaf, µs, never held across an await; the auth layer and the handler each take and release it independently (the handler re-verifies rather than trusting that a layer ran). Added 2026-09-18. |
| 39 | `FederationClient::state` | `std::sync::Mutex<ClientState>` (link + pool + resolver + last catalogue) | `connect` (after the HTTP reply), `call` (section 1: admit link, resolve, take a slot; section 2: release the slot), `silent`, `disconnect`, `revoke`, `link_state`, `last_catalogue` (`src/federation/client.rs`, item 2 PR 8) | Leaf, µs; **never held across the HTTP await by construction** — `call` is two critical sections around it. Added 2026-09-18. |
