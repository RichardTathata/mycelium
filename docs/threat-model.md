...
- *Boundary H mitigations in force (2026-09-24; plan [`plans/boundary-h.md`](plans/boundary-h.md), milestones M1–M2):*
  ...
  - **A1, authority at execution** (#398; ADR `design/authority-at-execution.md`). `ExecutionGate::check`
    ...
    **Wired at the gateway** (#399): a grant presented in `params._meta.mandate` by the **authenticated** caller,
    ...
    runs). *Not covered:* `llm.invoke` at the provider. **Every door, tried** (C7): after a revocation, `/mcp`, `/a2a` (send and stream), the raw routes, a member's direct call and the SDK serve stream each run nothing, by the handlers' own counters (`test_c7_the_bypass_matrix_no_door_runs_revoked_work`). **Running work stops too** (C10): a periodic sweep re-checks work admitted under a mandate and cancels it when the mandate lapses (MCP handlers are dropped; `rpc_rx` loops are cooperative); SDK-served work is not cancelled. *Limits:* other resources
    apply it only when wired;
    a restarted reader refuses checkpoints issued before it started, and installed epochs are journalled (closure plan C8), which rests on the authority issuing cumulative checkpoints; the wiki store checks before its git transaction, so a paused process can write late by the pause (C9); timing is measured on a live node (C12, `examples/authority_drain`) and across pods in the confined-fleet kind job, where the pods share one clock, so *s* is declared there rather than exercised. *Gate:* `mandate::authority::tests`, including both clock extremes.
