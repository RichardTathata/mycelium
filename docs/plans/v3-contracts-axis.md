...
| **CN2** ◐ *(2026-09-18)* | CN1 · 6·PR2 kernel | the award replays; a double-award witness (award rule removed) fails under replay. *Landed:* `commit_award_linearizable` — the award through a consensus round on `cn/{req}/award`, `Superseded` → `AlreadyAwarded` with the committed award, a timed-out round `AwardUnknown`; the witness as an **explicit interleaving** (two declarers plan, then commit: the plain KV path lets both commit and LWW keeps one; the linearizable path refuses the second). *Not landed, pinned as a gap:* the replay — a whole-node recording diverges on the interleaving of a governor's rng draw and the round's defer timer (inventory §2.3, the scheduler seam); the test asserts the divergence so the seam's arrival flips it | witness fails, real rule passes — **met by explicit interleaving, not under the kernel** |
