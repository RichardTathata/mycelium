...
### C5: Cutting a member off (L; needs its own ADR and review) — **implemented, see §8**
...
| C7 | #419 | **In-process, not yet in a deployment.** One node, both gateway and provider, with enforcement and an authority; every door the code has is tried after revocation, and the pass condition is the handlers' own counters. **Out of the matrix, gated where built:** federation calls (the same `ae_preflight`; `federation_transport` tests), the wiki store (`git_store_authority`), and reconnecting as a removed member (C5, `member_removal`). **The deployment variant** waits on the node image C12 needs **Deployment variant: this PR.** Phase 2 of the confined-fleet job runs the node image as an authority, a provider and the gateway; from the agent pod, the plant reaches the provider's handlers across pods, and after revocation `/mcp`, `/a2a` (send, stream) and the raw routes run nothing (the provider's counters), and the member plane is unreachable from the agent pod (with a positive control). The SDK serve stream and a member's own direct call are not re-run in the cluster: agents cannot reach the member plane there, and both stay gated in process |
| C12 | #420 | **Measured on a live node, not yet in the kind cluster.** `examples/authority_drain` (run in CI) admits calls under a mandate on a real node, revokes it, and checks T_admit and T_drain from the node's own records against the declared bound; a first local run gave T_drain = 25 ms against a bound of 300 ms, T_admit none, no unconfirmed stops. **Open:** running it inside the confined-fleet kind job needs a node image built in that job; the deployment variants of C7 and C12 wait on it. **Found:** an MCP tool loop serves one call at a time, so queued calls meet the revocation at admission. **Found:** Rust had no public `arguments_digest` (the SDKs did); now `mycelium::arguments_digest`, pinned to the same golden vector **Deployment variant: this PR.** In the kind cluster, calls admitted through the gateway to a provider pod, revoked by an authority pod: first local run T_admit none, T_drain 117 ms against the declared 300 ms plus 500 ms checkpoint delivery (members poll the authority), every stop confirmed. One kind node, one clock: *s* is declared, not exercised |
| C5 | #421 | **Built as the ADR was adopted** (`docs/design/member-removal.md` §7). **Configured by call** (`with_membership_authorities`), not a config field. **A removed node's signatures read as `Revoked`**, not a new variant: its keys join the revoked set, which is also what makes A1 refuse its mandates. **The confinement line is `ca_key_off_node`** (`Set` = the strict profile). **Not covered:** gateway tokens issued to the removed member's agents, which are revoked in the gateway's token configuration |
