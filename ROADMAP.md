...
> **v2.6.0** (2026-09-16) makes the AE gateway slice evidential — decisions and executions recorded to a node-local journal, only a hash-bearing reference in the gossiped chain, `/a2a` enforced, a live stale-policy check, and a cursor reader for exporters. Wire v12 unchanged; inert without an attached evaluator.
